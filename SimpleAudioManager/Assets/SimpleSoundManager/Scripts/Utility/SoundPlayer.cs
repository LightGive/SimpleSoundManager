	...
	public class SoundPlayer : MonoBehaviour
	{
		...
		void Update()
		{
			if (playType != PlayType.Random)
				return;
			waitTimeCnt += Time.deltaTime;

			if (waitTimeCnt > waitTime)
			{
...
namespace LightGive
{
	...
	public class SoundPlayerEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			{
				randomWaitTimeMinProp.floatValue = Mathf.Clamp(EditorGUILayout.FloatField("Wait min time", randomWaitTimeMinProp.floatValue), 0.0f, float.MaxValue);
